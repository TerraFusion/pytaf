...
npix = np.arange(n_src, dtype=np.int32).reshape((sy,sx))
sd = np.arange(n_src, dtype=np.float64).reshape((sy,sx))

# For some reason, passing 1-d generates memory error.
trg_data = pytaf.resample_s_g(modis_lat, modis_lon, 
                       y, x, modis_data, max_r, sd, npix)
print(trg_data)
# trg_data = pytaf.resample(modis_lat, modis_lon,
#                           latd, lond,
#                          modis_data, max_r, True, sd, npix)
